<template>
    <div>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark ">
            <a class="navbar-brand" href="#">{{archive_stat == 'yes'?'Job Posts Archive':'Job Posts'}}</a>
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav mr-auto">
                    <li class="nav-item active">
                        <button v-if="archive_stat != 'yes'" class="nav-link btn" type="button" data-toggle="modal" data-target="#modalCreatePost">
                            <i class="fa fa-plus-circle" aria-hidden="true"></i> Post Job
                        </button>
                    </li>
        ...
        <post-create-modal :cities='cities'></post-create-modal>
        <!--search bar-->
                            ...
                            <div class="col-lg-12" v-for='(post,index) in posts' :key='"post"+index'>
                                ...
                                <div class="card-header">
                                    ...
                                    <div class="media-body">
                                        <h5 class="mt-0"><small class="text-muted">{{parseInt(links.current_page) * 10 - 10 + (index+1)}}</small>. Title : <a :href='"/company/job_post/"+post.id' target="__blank" >{{post.title}}</a></h5>
                                        <p>
                                    ...
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                            ...
                                            <a :href='"/company/job_post/"+post.id' target="__blank" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            <button @click="archive(post.title,post.id)" type="button" class="btn btn-secondary">{{archive_stat == 'yes'?'Recover':'Archive'}}</button>
                                    </div>
                                    </div>
                                </div>
                                <div class="card-body">
